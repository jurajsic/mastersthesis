...
\thesissetup{
    ...
    bib           = example.bib, %bibliography.bib
...
In this section I will give an overview of existing solvers which tackle the problem. The ... of solvers can be found in ... and in .... that is why I will give only a short overiew of some solvers (maybe name them here). However I will explain into more details solver HQS, because the methods we used for developing our solver are based on the workings of this solver.
%current solutions, that already exist

%nieco o tom ze vacsina funguje na CNF, ale HQS a moje bude na NNF

%fast DQBF refutation - ze premienaju na QBF abo daco

\subsection{First Solution - DQDPLL}
The first solver~\cite{DPLLalgorithm} that tackled the satisfiability problem for DQBF was based on DPLL(TODO cite) algorithm which is succesfully used for BF and QBF solvers(TODO cite??). This algorithm works on formulas in CNF by  recursively trying to set variables to either true or false while checking if the disjuncts in CNF stay consistent. ??popisat nejak lepsie??? An adaptation called DQDPLL was introduced for DQBF, however it did not result in competitive solver.

\subsection{iDQ??}
The first ...dostatocne silny??efficient??.. DQBF solver (called iDQ(cite)) is based on the fact that Effectively Propositional Logic (cite) (EPR) is also NEXPTIME-complete, which means there exists a polynomial reduction from DQBF to EPR. The iDQ solver can then use techniques used for solving EPR logic but reduced to the more specific case of DQBF.
