...
\begin{definition}
  Let $V$ be a set of variables. A \emph{quantifier tree} \emph{over $V$} is a rooted directed tree in which each non-terminal node is assigned to a DQBF prefix and operation ${\diamond} \in \set{{\land},{\lor}}$ and each terminal node to a literal from $V$ (a variable $v \in V$ or its negation).
\end{definition}
