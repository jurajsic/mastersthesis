...
Last but not least in~\cite{HQSquantifierLocalization} the authors have given a theorethical foundations for non-prenex DQBFs and the possibility to push quantifiers inside formulas as explained in section ????. They also showed how to remove existential quantifiers inside formulas.????????????

\section{Preprocessing -- HQSpre}
