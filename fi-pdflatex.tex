...
\end{itemize}
\end{definition}
TODO: substitucia (i.e. $\phi\substitute{x}{0}$)
...
\end{itemize}
Each $\psi \in \BF{V}$ represents a \emph{Boolean function} $f_{\psi}\colon \valtns{V} \mapsto \set{0,1}$ over $V$ where $f_{\psi}(v) = v(\psi)$. The set of all Boolean functions over $V$ is denoted by $\BFuncs{V}$.
...
\label{sec:BF:NNF}
We say that BF $\phi$ is in negation normal form (NNF) if negation occurs only in front of the variable. Each BF $\phi$ can be easily transformed into an equivalent BF $\phi'$ by applying these two rules:
\begin{itemize}
    \item $\neg(\phi_1 \land \phi_2) = \neg\phi_1 \lor \neg\phi_2$ 
    \item $\neg(\phi_1 \lor \phi_2) = \neg\phi_1 \land \neg\phi_2$.
\end{itemize}
...
Again, we define satisfiability. Usually satisfiability for QBFs is defined trough valuation as in BFs case where evaluation of QBFs follow the same rules as BFs with two newly added rules for quantifiers:
\begin{itemize}
    \item 
\end{itemize}
We follow different but equal definition based on skolemization??? ??which is better when we want to define DQBFs???? A QBF $\psi$ from the definition is satisfiable if for each $x \in V_{\psi}^{\exists}$ there exist a function $s_{x}\colon x \mapsto V_{\psi}^{\forall}$ (called Skolem functions) such that $\phi$ where every $x \in V_{\psi}^{\exists}$ is replaced by $s_x(x)$ is evaluated to $1$ in every valuation $v\colon V_{\psi}^{\forall} \mapsto \set{0,1}$.
...
QBFs in PNF can be seen as a special case of DQBFs in PNF where the dependency sets are linearly ordered. That is for each two dependency sets $D_{y_1}$ and $D_{y_2}$ in QBFs it holds either $D_{y_1} = D_{y_2}$ or $D_{y_1} \subseteq D_{y_2}$ or $D_{y_2} \subseteq D_{y_1}$.
...
With the example in mind, we define satisfiability for DQBF in PNF similarly to how satisfiability is defined for QBFs. 
...
\begin{figure}[p]
    ...
    %\begin{centering}
    \begin{tabular}{cccc}
        rule & $V_{.}^{\exists}$ & $V_{.}^{\forall}$ & $V_{.}^{\text{free}}$\\\hline
        {\begin{prooftree}
            \hypo{v \in V}
            \infer1{v \in \DQBF{V}}
        \end{prooftree}}
        ...
        \end{prooftree}}
        & $V_{\psi_1}^{\exists} \cup V_{\psi_2}^{\exists}$ & $V_{\psi_1}^{\forall} \cup V_{\psi_2}^{\forall}$ & $V_{\psi_1}^{\text{free}} \cup V_{\psi_2}^{\text{free}}$\\[5mm] 
        {\begin{prooftree}
            \hypo{\psi_1 \in \DQBF{V}}
            \hypo{\psi_2 \in \DQBF{V}}
            \hypo{(\ref{disconjrule})}
        ...
        {\begin{prooftree}
            \hypo{\psi \in \DQBF{V}}
            \hypo{v \not\in \evars{\psi} \cup \uvars{\psi}}
            \hypo{(\ref{existrule})}
            \infer3{\exists v(D_v):\psi^{-v} \in \DQBF{V}}
        \end{prooftree}}
        & $\evars{\psi} \cup \set{v}$ & $\uvars{\psi}$ & $\fvars{\psi}$ \\[5mm]
        {\begin{prooftree}
            \hypo{\psi \in \DQBF{V}}
            \hypo{v \not\in \evars{\psi} \cup \uvars{\psi}}
            \infer2{\forall v:\psi \in \DQBF{V}}
        \end{prooftree}}
        & $\evars{\psi}$ & $\uvars{\psi} \cup \set{v}$ & $\fvars{\psi}$\\[5mm]
    \end{tabular}
    %\end{centering}
    ...
    \end{equation}
    \caption{Rules defining the syntax of DQBFs in NNF}
    \label{fig:DQBFrules}
\end{figure}

\begin{definition}
  Let $V$ be the set of variables. The set $\DQBF{V}$ of \emph{DQBFs in negation normal form} (NNF) over $V$ as well as their existential, universal and free variables are defined by Figure~\ref{fig:DQBFrules} The set $\DQBF{V}$ is defined to be the smallest set satisfying these rules. We set $V_{\psi} = \evars{\psi}\cup\uvars{\psi}\cup\fvars{\psi}$.
\end{definition}
...
TODO: substitution (ze nahradzujem aj inside dependency sets???? - mozno nie, mozno len odkaz pri theorem o localization)
...
%quantifier localization + dokaz o tom ze vieme to robit ako podformule (pre ten velky theorem kde to nezrobili, pre ten dalsi to netreba)

The first two theorems show when it is possible to push (or extract) quantifiers inside the formula. The first theorem is from~\cite[Theorem 3]{HQSquantifierLocalization} where ... is replaced by ... from~\cite[Theorem 4]{HQSquantifierLocalization}
...
\begin{theorem}[{\cite[Theorem 3,4]{HQSquantifierLocalization}}]
  ...
  \begin{align}
      ...
      \forall x : (\psi_1 \diamond \psi_2) &\approx (\psi_1^{-x} \diamond (\forall x : \psi_2)) \text{, if } x \not\in V_{\psi_1} \\
...
??????
\begin{theorem}
  Let $\psi \in \DQBF{V}$ and $\psi_1, \psi_2$ are the formulas from previous theorem on ???each?? side of the same equality???? where $\psi_1$ is subformula of $\psi$. Again, we assume that $x'$ and $y'$ are fresh variables which do not occur in $\psi$. Then $\psi$ is equisatisfiable to $\psi'$ which results from $\psi$ where $\psi_1$ is replaced by $\psi_2$.
\end{theorem}
??????

%quantifier elimination
%\begin{itemize}
%    \item universal expansion + dokaz ze vieme robit ako podformule
%    \item existential elimination - len povedat a odkazat sa na clanok kde je dokaz
%\end{itemize}
...
\begin{theorem}[Universal Expansion]
  Let $\psi, \psi_1 \in \DQBF{V}$ such that $\forall x : \psi_1$ is a subformula of $\psi$. Let $\psi_2$ be a DQBF that results from $\psi_1$ by substituting each $y \in V_{\psi_1}^{\exists}$ such that $x \in D_y$ with a new variable $y' \not\in V_{\psi}$ where $D_{y'} = D_y$. Then $\psi \approx \psi'$ where $\psi'$ results from $\psi$ by replacing the subformula $\forall x : \psi$ by $\psi_1^{-x}\substitute{x}{0} \land \psi_2^{-x}\substitute{x}{1}$.
\end{theorem}
\begin{example}
