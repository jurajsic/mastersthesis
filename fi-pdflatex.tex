...
To make these rules more usable for DQBF in NNF we have to show that they also work while replacing subformulas. This was not proven by Ge-Ernst et al.~\cite{HQSquantifierLocalization} even though they used it for replacing subformulas. As we will show, this was used erroneously because the rule~(\ref{loc:exdisj}) does not generally hold. However, for other rules there is no problem as the next theorem shows. % that if DQBFs from the previous theorem occur as subformulas of DQBF $\psi$ they can be replaced without changing the satisfiability of $\psi$. 

\begin{theorem}
  \label{thrm:quantLocalSubformulas}
  Let $\psi \in \DQBF{V}$, $\phi_1$ be the left side formula and $\phi_2$ be the right side formula  from the same equality from Theorem~\ref{thrm:quantLocalEq} (except~(\ref{loc:exdisj})) where $\phi_1$ is a subformula of $\psi$. Again, the same assumptions for $x'$ and $y'$ variables hold as in Theorem~\ref{thrm:quantLocalEq} but also they do not occur in $\psi$. Then $\psi \approx \psi'$ where $\psi'$ results from $\psi$ by replacing $\phi_1$ with $\phi_2$.
\end{theorem}
\begin{proof}
...
As we already mentioned, the rule~(\ref{loc:exdisj}) as it stands cannot be generally used for replacing subformulas. We show it on the following formula:
\[\forall x \exists y(\emptyset) \itholds ((x \land y) \lor (\neg x \land \neg y)).\]
It is obviously not satisfiable because whether $s(y) = 0$ or $s(y) = 1$ we always can find valuation in which both disjuncts are not true. However if we assumed that rule~(\ref{loc:exdisj}) can be used also for subformulas we would get
\[\forall x \itholds (\exists y(\emptyset) \itholds (x \land y) \lor \exists y'(\emptyset) \itholds (\neg x \land \neg y'))\]
which is satisfiable for $s$ where $s(y) = 1$ and $s(y') = 0$. To fix this, we need to restrict the variables that can occur in the disjuncts. As the next theorem shows, it is enough that universal variables that occur in both disjuncts (including in dependency sets) are only those from the dependency set of $y$.

This shit I am writing here does not work for \[\forall x_1 \forall x_2 \itholds (\exists y(\emptyset) \itholds (x_1 \land y) \lor (x_2 \land y)) \lor (\neg x_1 \land \neg x_2)\]
\begin{theorem}
  Let $\psi,\psi_1,\psi_2 \in \DQBF{V}$ such that $\exists y(D_y) \itholds (\psi_1 \lor \psi_2)$ is a subformula of $\psi$. Let 
  \[A_{\psi_1} = \Set{x \in \uvars{\psi} | x \in \vars{\psi_1} \text{ or } x \in D_{y_1} \text{ for some } y_1 \in \vars{\psi_1}}\]
  and 
  \[A_{\psi_2} = \Set{x \in \uvars{\psi} | x \in \vars{\psi_2} \text{ or } x \in D_{y_2} \text{ for some } y_2 \in \vars{\psi_2}}\] 
  where $A_{\psi_1} \cap A_{\psi_2} \subseteq D_y \cup \fvars{\psi}$.
  ...
  Let $y'$ be a new variable not occurring in $\psi, \psi_1$ or $\psi_2$. Then $\psi \approx \psi'$ where $\psi'$ results from $\psi$ by replacing the subformula $\exists y(D_y) \itholds (\psi_1 \lor \psi_2)$ by \[(\exists y(D_y) \itholds \psi_1) \lor (\exists y'(D_y) \itholds \psi_2\substitute{y}{y'}).\]
\end{theorem}
\begin{proof}
   First, the assumption $A_{\psi_1} \cap A_{\psi_2} \subseteq D_y \cup \fvars{\psi}$ tells us that for any two $s,s' \in \Scands{\psi}$ it holds that $s(\psi_1) \cap s'(\psi_2) \subseteq D_y$. Now, let $\phi_1 = \exists y(D_y) \itholds (\psi_1 \lor \psi_2)$ and $\phi_2 = \exists y(D_y) \itholds \psi_1) \lor (\exists y'(D_y) \itholds \psi_2\substitute{y}{y'})$. We prove that $\seman{\psi} \not= \emptyset$ iff $\seman{\psi'} \not= \emptyset$.
  ...
  Now assume that $s' \in \seman{\psi'}$. We will prove that for some $s$, where $s(y)$ is a combination of  $s'(y)$ and $s'(y')$, and $s(v) = s'(v)$ for other variables, is a satisfying Skolem functions mapping of $\psi'$. Let $v \in \valtns{\psi'}$ be a valuation and $v_{D_y}$ its restriction to $D_y$. With similar reasoning as in the proof of case~(\ref{loc:unconj1}) of Theorem~\ref{thrm:quantLocalSubformulas} we only need to show that $v(s(\phi_1)) = 1$ assuming that $v(s'(\phi_2)) = 1$. From this assumption we can infer that $v(s'(\psi_1)) = 1$ or $v(s'(\psi_2\substitute{y}{y'})) = 1$. Without loss of generality assume that $v(s'(\psi_1)) = 1$. Then we set $s(y)(v_{D_y})$ to $s'(y)(v_{D_y})$ which means that $v(s(\psi_1)) = 1$, therefore $v(s(\phi_1)) = 1$. However, we need to show that for different valuation $v' \in \valtns{\psi'}$ such that $v'(x) = v(x)$ for $x \in D_y$ (therefore $v_{D_y}' = v_{D_y}$) it also holds that $v'(s'(\psi_1)) = 1$ (and so $v'(s(\phi_1)) = 1$). Assume now it does not hold. For $v'(s'(\phi_2))$ to be equal to $1$ it would have to hold then that $v'(s'(\psi_2\substitute{y}{y'})) = 1$. There needs to be a valuation with restriction to $D_y$ equal to $v_{D_y}$ such that it evaluates $s'(\psi_2\substitute{y}{y'})$ to $0$, otherwise we could set $s(y)(v_{D_y})$ to $s'(y')(v_{D_y})$. For simplicity, assume that $v$ is such a valuation, therefore $v(s'(\psi_2\substitute{y}{y'})) = 0$. This means that we can create valuation $v''$ where we set variables from $s'(\psi_1)$ to the same value as $v$ and variables from $s'(\psi_2\substitute{y}{y'})$ to the same value as $v'$ (others do not matter). This is possible because variables occurring both in $s'(\psi_1)$ and $s'(\psi_2\substitute{y}{y'})$ are only those that are in $D_y$ (see the beginning of the proof) and $v_{D_y} = v_{D_y}'$. However, in this valuation it holds that $v''(s'(\psi_1)) = v''(s'(\psi_2\substitute{y}{y'})) = 0$ which means that $v''(s'(\phi_2)) = 0$ which is contradiction .... with what... this is not true, it does not have to be equal to 1, it is only subformula......
\end{proof}
...
\begin{algorithm}
  ...
  \begin{algorithmic}[1]
    ...
    \State (DQBFprefix, $\phi$) = \Call{TurnToPrefixAndBDD}{$r$}
...
In this section we give a ... of the function \textproc{Localise} from Algorithm~\ref{alg:general}. We use two ...implementations... of this algorithm: one that basically does nothing, which means that after doing \textproc{TurnToPrefixAndBDD}, the resulting quantifier prefix will be the same as the quantifier prefix of the input formula $\psi$ and another that actually pushes quantifier inside. This second ???implementation??? is based on the algorithm which is used for localising quantifiers in the solver HQS~\cite{HQSquantifierLocalization}.
%\begin{itemize}
...
Algirithm~\ref{alg:localise} shows this implementation of localising. Function \textproc{Localise} first, based on the assigned operator to node $n$ either calls \textproc{LocaliseOR} or \textproc{LocaliseAND}. These functions push variables to the children based on the rules from Theorem~\ref{thrm:quantLocalEq}. After that it recursively runs \textproc{Localise} on the children of $n$. Notice that this algorithm does not do anything for terminal nodes. 
...
\begin{algorithm}[ht]
  ...
  \begin{algorithmic}[1]
    \Function{LocaliseAND}{n}
      \While{$\prefix{n} \cap \evars{n} \not= \emptyset$}
        \State $y \gets \Call{GetNextExistVarToPush}{\null}$
        \State $yChildren \gets \set{n' \in \children{n} | y \in \evars{n'}}$
        \If{$yChildren = \children{n}$}%\Comment{$y$ is in every child}
          \State break
        \Else
          \State $n_y \gets \Call{CombineChildren}{yChildren, {\land}}$
          \State $\children{n} \gets (\children{n} \setminus yChildren) \cup n_y$
          % TODO, explain that even though we do not rename, the algorithm still work, also explain what pushing means
          \State \Call{PushVar}{$y$, $n_y$}
          \State remove $y$ from $\prefix{n}$
        \EndIf
      \EndWhile
      %\Statex \(\triangleright\) get all univ vars on which none of the exist vars in quantifier prefix depends
      \State $pushableUVars \gets \set{x \in \prefix{n} \cap \uvars{n} | x \not\in D_y \text{ for all } y \in \prefix{n} \cap \evars{n}}$
      \ForAll{$x \in pushableUVars$} 
        \ForAll{$n' \in \children{n}$}
          \If{$x \in V_{n'}$}
            \State \Call{PushVar}{$x$, $n'$}
          \Else
            \State remove $x$ from dependencies of all $y \in \evars{n'}$
          \EndIf
        \EndFor
        \State remove $x$ from $\prefix{n}$
      \EndFor
...
\begin{itemize}
    ...
    \item eliminate only those that are not in support set -- useless, it will basically do what nothing does, because we assume that input formula has already only those variables in quantifier prefix which are in support set, just pushing it inside and then taking them out will not delete anyone --- it might delete some dependecies though (but nothing should do same, if we assume that nothing is doing localising)
    \item eliminate only exist vars (or maybe just some univ. vars - those that only few exist vars depend on (probably 0, maybe 1 or 2) or those that will allow me to remove some exist vars)??? - basically eliminate only when no (a few) new variables are created
...
\begin{itemize}
    ...
    \item remove based on leftover variables if universal expansion would be done, i.e. if are thinking about removing $x$ in formula $\forall x \itholds \psi$, then we check how much variables are in $V_{\psi\substitute{x}{0}} \cup V_{\psi\substitute{z}{1}}$ and we choose that universal variable for which this number is lowest
