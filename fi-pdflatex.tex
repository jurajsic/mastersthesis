...
\subsection{iDQ??(+iProver??)}
The first ??efficient?? DQBF solver (called iDQ(cite)) is based on instantiation technique used for solving Effectively Propositional Logic (cite). This solver also works on formulas in CNF where in each step of the algorithm it tries to create a BF that is an overapproximation of input DQBF by instanciating each clause .... This BF is then checked if it is satisfiable, if it is not, then the input DQBF is not satisfiable. If it is, they check whether resulting valuation is valid for the input DQBF. If it is not, then it is used to create more clause instances which are then used to refine this overapproximation.
...
\subsection{dCAQE}
zalozene na abstraction refinement (viacmenej iDQ je tiez abstraction refinement)
