...
We first start with defining Boolean formulas. For these we need a set of variables $V = \set{x_1, \dots, x_n}$ for which we can define a valuation function $v\colon V \mapsto \{0,1\}$ which says whether each variable is true or false. Now we can define Boolean formulas as variables connected using logical operatives \emph{and} (${\land}$), \emph{or} (${\lor}$) and \emph{negation} (${\neg}$).
...
\begin{itemize}
    ...
    \item $\neg\phi$, where $\phi$ is Boolean formula.
\end{itemize}
\end{definition}
Let $BF(V)$ be a set of all Boolean formulas over $V$ and $v\colon V \mapsto \set{0,1}$ a valuation over the set of variables $V$. We can extend this valuation to $BF(V)$ as $u\colon BF(V) \mapsto \{0,1\}$ where
\begin{itemize}
    \item $u(x_i) = v(x_i)$ for $x_i \in V$,
    \item $u(\phi_1 \land \phi_2) = 1$ if both $u(\phi_1) = 1$ and $u(\phi_2) = 1$, otherwise $u(\phi_1 \land \phi_2) = 0$,
    \item $u(\phi_1 \lor \phi_2) = 1$ if either $u(\phi_1) = 1$ and $u(\phi_2) = 1$, otherwise $u(\phi_1 \lor \phi_2) = 0$ and
    \item $u(\neg\phi) = 1$ if $u(\phi) = 0$ otherwise $u(\neg\phi) = 0$.
\end{itemize}
...
tu dat nonprenex ak robim, semantika + priklady + dokazy o nahradzovani

\section{Binary Decision Diagrams}


\chapter{The Problem and Current Solutions????}
