...
\label{thrm:quantLocalEq}
  ...
  \begin{align}
      ...
      \forall x \itholds (\psi_1 \land \psi_2) &\approx (\forall x \itholds \psi_1) \land (\forall x' \itholds \psi_2\substitute{x}{x'}) \tag{a} \label{loc:unconj1} \\
      ...
      \forall x \itholds (\psi_1 \diamond \psi_2) &\approx \begin{multlined}[t][3cm](\psi_1 \diamond (\forall x \itholds \psi_2)) \text{, if } x \not\in V_{\psi_1} \tag{c} \label{loc:undisj} \\
      \text{ and } x \not\in D_y \text{ for all } y \in \evars{\psi_1}\end{multlined} \\
      \exists y(D_y) \itholds (\psi_1 \lor \psi_2) &\approx (\exists y(D_y) \itholds \psi_1) \lor (\exists y'(D_y) \itholds \psi_2\substitute{y}{y'}) \tag{d} \label{loc:exdisj} \\
...
which is not satisfiable, we have that $x_1 \land y$ contains $x_1$ which is not in $x_2 \land y$ and $x_2$ is only in the second disjunct. However, after applying the rule~\eqref{loc:exdisj} we get
\[\forall x_1 \forall x_2 \itholds ((\exists y(\emptyset) \itholds (x_1 \land y) \lor \exists y'(\emptyset) \itholds (x_2 \land \neg y')) \lor (\neg x_1 \land \neg x_2)),\]
which becomes satisfiable. The problem here is that $x_1$ and $x_2$ are in $\neg x_1 \land \neg x_2$ which is outside the subformula on which we are applying the rule $\eqref{loc:exdisj}$.



the intersection of universal variables (including those on which existential are dependent on) of both disjuncts  As the next theorem shows, it is enough that universal variables that occur in both disjuncts (including in dependency sets) are only those from the dependency set of $y$.
...
\begin{theorem}
  ...
  %where $A_{\psi_1} \cap A_{\psi_2} \subseteq D_y$.
...
\end{theorem}
\begin{proof}
   First, the assumption $A_{\psi_1} \cap A_{\psi_2} \subseteq D_y \cup \fvars{\psi}$ tells us that for any two $s,s' \in \Scands{\psi}$ it holds that $s(\psi_1) \cap s'(\psi_2) \subseteq D_y$. Now, let $\phi = \exists y(D_y) \itholds (\psi_1 \lor \psi_2)$ and $\phi' = \exists y(D_y) \itholds \psi_1) \lor (\exists y'(D_y) \itholds \psi_2\substitute{y}{y'})$. We prove that $\seman{\psi} \not= \emptyset$ iff $\seman{\psi'} \not= \emptyset$.
