...
%TODO fix this with new chapter
...
\chapter{Quantifiers Manipulations}
...
%We first prove a lemma that says that if we have subformula   such that no negation occurs before it, we do not care ???
%\begin{lemma}
%  \label{thrm:???}
%  Let $\phi, \phi_1, \phi_2 \in \BF{V}$ where $\phi_1$ is a subformula of $\phi$ and for each subformula $\neg \phi'$ of $\phi$ we have that $\phi_1$ is not subformula of $\phi'$ Let $v \in \valtns{V}$ where $v(\phi) = v(\phi_2) = 1$ and $v(\phi_1) = 0$.
%\end{lemma}
%\begin{proof}
%  TODO
%\end{proof}
...
\begin{proof}
  ...
  %This means that we can focus only on the case where $v(s(\phi)) = 1$, otherwise the value of $v(s(\psi))$ would not depend on the subformula $s(\phi)$. 
...
\begin{proof}
  ...
  Now, assume that $s' \in \seman{\psi'}$. For the rest of the proof, for every valuation $v \in \valtns{\uvars{\psi}}$ (where it holds that $v \in \valtns{\uvars{\psi'}}$, because $\uvars{\psi} = \uvars{\psi'}$) we use $v_{D_y}$ to denote the restriction of $v$ to $D_y$. Let $s$ be a Skolem function mapping of $\psi$ such that for $v \in \valtns{\uvars{\psi}}$ %$v_{D_y} \in \valtns{D_y}$ we have 
  \begin{itemize}
