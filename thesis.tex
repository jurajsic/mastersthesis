...
\thesissetup{
    ...
    keywords      = {dependency quantified Boolean formula, DQBF, binary decision diagram, BDD, satisfiability},
    TeXkeywords   = {dependency quantified Boolean formula, DQBF, binary decision diagram, BDD, satisfiability},
    abstract      = {%
...
After this, we define BDDs and recall some of their properties. Finally, we describe quantifier trees which can represent DQBFs.
...
\begin{figure}[htp]
    ...
    \end{equation}
    \caption{The rules defining the syntax of DQBFs in NNF (based on Ge-Ernst et al.~\cite{HQSquantifierLocalisation}).}
    \label{fig:DQBFrules}
\end{figure}
...
\begin{figure}[t]
    ...
    \begin{subfigure}[b]{.5\linewidth}
        ...
        \label{fig:ROBDD}
    \end{subfigure}
    \caption{An example of BDD and ROBDD representing the same BF} %$((\neg x_1 \land x_2 \land x_3) \lor (x_1 \land \neg (x_1 \lequal x_2))$}
...
\end{enumerate}
They have the nice property that for a given variable order, each BF is represented by exactly one ROBDD. However, the size of two ROBDDs for the same formula can be very different. We can find a formula which in one ordering has a ROBDD representation whose number of nodes is linear to the size of the formula and in a different one it is exponential. The problem of finding the optimal variable ordering is $\NP$-hard, therefore we usually use some heuristics to reduce the size of BDDs and we do not look for the perfect ordering.
...
For each operation on BFs (${\land}$, ${\lor}$, ${\neg}$), there is an algorithm on ROBDDs which can get the resulting ROBDD representing the application of the operation. For example, if we have a ROBDD representing BF $\psi_1$, we can get a ROBDD representing $\neg \psi_1$ by swapping terminal nodes $0$ and $1$. See for example Andersen~\cite{BDDdetails} for more details.

In the following chapters, when we use BDD we mean ROBDD.
...
In this section we give an explanation of function \textproc{Localise} from Algorithm~\ref{alg:general} which uses rules from Theorem~\ref{thrm:quantLocalEq} to push quantifiers as deep inside a formula as it is possible.
...
\begin{figure}[ht]
  ...
  \end{forest}}
  \par\vspace{0.4cm}
  \caption{After transforming ${\land}$ nodes}
  \label{fig:eliminate:terminals}
  \end{subfigure}
  \par\bigskip
  \begin{subfigure}{0.48\textwidth}
  \centering
  \scalebox{0.7}{\begin{forest}
    [$\forall x_2$
      [${\lor}$, circle, draw, minimum size=2em, inner sep=1pt
        [$x_1 \lor x_2$, edge label={node[midway,above left]{$\forall x_1$}}]
        [$x_2 \land y_2$, edge label={node[midway,above right]{$\exists y_2(\emptyset)$}}]
      ]
    ]
  \end{forest}}
...
This chapter shows the experimental results of different versions of DQBDDs and its comparison with different DQBF solvers (iDQ v1.0\footnote{\url{http://fmv.jku.at/idq/}}, iProver v3.1\footnote{\url{http://www.cs.man.ac.uk/~korovink/iprover/}}, dCAQE v4.0.1\footnote{\url{https://github.com/ltentrup/caqe/}} and HQS with quantifier localisation\footnote{\url{https://abs.informatik.uni-freiburg.de/src/projects_view.php?projectID=21}}). All our benchmarks were run on ??6 cores???  with ...?????????..... using BenchExec v2.2~\cite{benchexec}, a framework for reliable benchmarking and rescource measurement, where we set per benchmark the runtime limit at \SI{15}{min} and the memory consumption limit at \SI{4}{GB}. We also run all benchmarks in parallel on 6 cores???
...
\paragraph{CSP}
This set consists of 461 instances of controller synthesis problem~\cite{CSPbenchmarks} in which we try to find safe controller for controllable transition system. Controllable transition system is composed of state and two types of inputs -- uncontrollable and controllable -- which decide to which state the system moves. Controller decides the controllable inputs and a safe controller is such a controller that the transition system always stays in some set of safe states.
...
To be able to compare DQBDD to other solvers it is important to choose the right heuristics for choosing the universal variables to be eliminated (see Section~\ref{sec:algelimination}) and quantifiers to eliminate in quantifier tree (see Section~\ref{sec:algturntobdd}). For this we compare these heuristics on E19 benchmark set as it is a selected combination of other benchmarks and it is reasonably small. 


\begin{table}[ht]
\centering
\caption{Comparison of DQBDD heuristics for the choice of the next universal variable to eliminate and which quantifiers to eliminate during transformation to BDD on E19 benchmark with the total number (\textbf{\#}) of instances solved and the number of satisfiable (\textbf{S}) and unsatisfiable (\textbf{U}) from these.}
\label{tab:heurcomp}
\begin{tabular}{|l||l|l|l|l|l|l|l|l|l|}
\hline
\multicolumn{1}{|c||}{\textbf{order of the next}} & \multicolumn{9}{c|}{\textbf{what quantifiers to eliminate}} \\ \hhline{~*{9}{-}} 
...
\textbf{set at beginning}    & 94 & 41 & 53 & 139 & 43 & 96 & 134 & 38 & 96 \\ \hline
\textbf{current lowest}      & 94 & 41 & 53 & 138 & 42 & 96 & 134 & 38 & 96 \\ \hline
\textbf{vars in conjuncts}   & 94 & 41 & 53 & 139 & 43 & 96 & 135 & 40 & 95 \\ \hline
\end{tabular}
\end{table}

Table~\ref{tab:heurcomp} shows this comparison where we run every possible combination of heuristics. It shows the number of solved instances (\textbf{\#}) and from these the number of \textbf{S}atisfiable and \textbf{U}n\-sa\-ti\-sfi\-able instances. It is obvious that it is best to eliminate only simple quantifiers (meaning all possible existential variables and only those universal variables which do not have any dependency) during the transformation to BDD. However, eliminating all universal variables during this transformation was not much worse compared to the eliminating simple ones, only a few less instances were solved. For the heuristic that chooses the next universal variable to eliminate, it is not obvious which one is the best. The number of solved instances here is pretty much the same for a chosen heuristic of which quantifiers to eliminate so the choice of which one is the best is not so clear. We can however compare the total runtimes of all solved instances. In the case of simple elimination, we get that the total runtime of solved instances by heuristic which sets the order of unviersal variables to eliminate at the beginning is \SI{3440.76}{s}, by heuristic which chooses the universal variable as the one with currently the lowest number of dependencies is \SI{3785.59}{s} and the one that chooses based on the number of variable in the two BDDs representing the conjuncts of universal expansion is \SI{4125.18}{s}. Obviously, from these we can see that it is best to eliminate simple quantifiers and set the order of universal variables at the beginning.

TODO: say something about vars in conjuncts is like current lowest, but not always, and then it is better (also for some cases, the number of dyn reorderings is probably so hugh -- probably caused by the constant checking of conjuncts -- that it can sometimes result in small BDD and then it solves some instances (2?) which other heuristics cannot.
...
\textbf{iProver} & \num{4} & \num{1706.29} & \num{3} & \num{1} & \num{0} & \num{18} & \num{0} & \num{0} \\ \hline
\end{tabular}
\caption{Solvers on SAT...}
\label{tab:SAT}
\end{table}

uplne pohorel, je to tym ze SAT problemy boli asi dostatocne tazke a nevedel vytvorit BDDcko ani v jednom pripade (alebo quantifier tree). Toto je well knows fact o BDDckach, ze they do not scale???
...
\textbf{iProver} & \num{43} & \num{4918.77} & \num{23} & \num{20} & \num{0} & \num{23} & \num{179} & \num{1} \\ \hline
\end{tabular}
\caption{Solvers on E19...}
\label{tab:E19}
\end{table}


\chapter{Conclusion}
We have implemented quantifier elimination DQBF solver DQBDD that uses BDDs. We have shown missing proofs used for quantifier localisation while fixing the disjunction problem??? as well as added proof for universal expansion for subformulas. We have done an extensive evaluation of different heuristics used in DQBDD and compared the best ones with other DQBF solvers. We have shown that our solver is da best so fuck off

As future work we are planning to???
\begin{itemize}
    \item it is a good idea to think whether HQS and DQBDD can be combined as in AIGsolve where they use AIGs but try to change them to BDDs when possible??? add BDDs to HQS similarly as BDDs are used in AIGsolve, especially during the localisation phase, it seems that the case where we did not localise (and so did not create quantifier tree) it never went out of memory, however it can during locaslisation when the tree is too big. This is because when we create just BDD, no tree, we do it from the gates and so the same gate represents the same BDD so we do not create it multiple times. But for the localisation algorithm to work properly, we have to create for each gate multiple same quantifier trees, because we might push stuff inside them in one branch but not in other. However, this might be too much, maybe it is better to just have tree share subtrees and when we see we need to push something, create a copy then??? This is a bit similar to how they do it in HQS, they basically transform AIG to quantifier tree sharing subtrees or something like that, but they never create copies, they push only if all parents share the quantifiers?? We could combine this somehow and do it like that, and then when we create the BDD, we create it up to some size or something, otherwise we keep it as AIG???
    \item It would be also good idea to add more preprocessing (PSPACE subset, approximations, using bloqqer with caqe) ?
    \item Another possible direction is to implement the quantifier elimination only of those universal variables which result in qbf and possibly design and implement a QBF solver using BDDs which could solve it???
    \item Finally, we should look into whether initializing variable order for BDDs makes sense (and what should it be) and what other reordering techniques could we use (we now use dynamic sifting, could be interesting to try others that CUDD implements, also would be interesting to use ''static'' reordering, where we would say when to reorder)
\end{itemize}
