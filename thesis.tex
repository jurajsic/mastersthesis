...
In this section, we give an explanation of function \textproc{Localise} from Algorithm~\ref{alg:general} which uses the rules from Theorem~\ref{thrm:quantLocalEq} to push quantifiers as deep inside a formula as it is possible.
...
Algorithm~\ref{alg:localise} shows the implementation of localising quantifiers. On a nonterminal node, function \textproc{Localise} calls either \textproc{LocaliseAND} (line~\ref{localise:calland}) or \textproc{LocaliseOR} (line~\ref{localise:callor}) according to the operation assigned to the node. These functions push variables from $\prefix{n}$ to the children of $n$ based on the rules from Theorem~\ref{thrm:quantLocalEq}. For terminal nodes representing a literal with a variable $x$, it uses rules \eqref{elim:univ} and \eqref{elim:exist} of Lemma~\ref{lemma:elimination} to remove all quantifiers which are not quantifying $x$. After that, the algorithm recursively runs \textproc{Localise} on the children of $n$.
