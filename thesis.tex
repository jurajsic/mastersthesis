...
To be able to compare DQBDD to other solvers it is important to choose the right heuristics for choosing the universal variables to be eliminated and quantifiers to eliminate in quantifier tree (see Section~\ref{sec:quantLocalisation} and \ref{sec:quantElimination}). For this we compare these heuristics on E19 benchmark set as it is a selected combination of other benchmarks and it is reasonably small. 
...
Table~\ref{tab:heurcomp} shows this comparison where we run every possible combination of heuristics. 

tell which is best, we use that for the next section
...
\begin{figure}
  ...
  \begin{tikzpicture}
    ...
    \addplot+[
      ...
      %mark=,
      %mark size=2.9pt]
      table[col sep=comma]{data/DQBDDsimpleallcomparison.csv};
    \end{axis}
  \end{tikzpicture}
  \caption{Caption}
  \label{fig:my_label}
...
To also show that it is better to use preprocessor and dynamic reordering, we run DQBDD (with simple quantifiers elimination and the order of the next variable set at beginning) without preprocessor and without dynamic reordering. The run without preprocessor solved only .... (with ... satisfiable and ... unsatisfiable) and the one without dynamic reordering solved only 84 instances (with 37 satisfiable and 47 unsatisfiable).
...
\begin{itemize}
    \item it is a good idea to think whether HQS and DQBDD can be combined as in AIGsolve where they use AIGs but try to change them to BDDs when possible??? add BDDs to HQS similarly as BDDs are used in AIGsolve
